...
In Azure Go SDK, profiles are available under the profiles/ path, with their version in the “YYYY-MM-DD” format. Right now, the latest Azure Stack profile version is “2017-03-09”. To import a given service from a profile, you need to import its corresponding module from the profile. For example, to import “Compute” service from “2017-03-09” profile:

```go
import "github.com/Azure/azure-sdk-for-go/profiles/2017-03-09/compute/mgmt/compute"
```

# Install Azure Go SDK on Azure Stack
...
3. Install Go SDK and its dependencies by running the following bash command:
    ...
    * Azure Go SDK uses Go-AutoRest modules to send requests to Azure resource provider endpoints. To use Azure services in code, in addition to Azure Go SDK, modules must be imported from [Go-AutoRest](https://github.com/Azure/go-autorest). More details about the most common Go-Autorest packages can be found [here](https://docs.microsoft.com/en-us/go/azure/azure-sdk-go-install#including-the-azure-sdk-for-go-in-your-project). For a complete list of the available modules in Go-AutoRest go [here](https://godoc.org/github.com/Azure/go-autorest).

# How to use GO SDK profiles on Azure Stack
To run a sample of Go code on Azure Stack:

1. Install Azure Go SDK and its dependencies (see previous section)
2. Get metadata information from resource manager endpoint. Resource manager metadata endpoint returns a JSON file with required information to run Go samples. Resource manager metadata endpoint is located at: `ResourceManagerURL`/metadata/endpoints?api-version=1.0
    * Note:
        * `ResourceManagerURL` in one node environment is: https://management.local.azurestack.external/
        * `ResourceManagerURL` in multi node environment is: https://management.`LOCATION`.ext-`MACHINE_NAME`.masd.stbtest.microsoft.com/'
...
6. In your function, create and authenticate a client with a New*Client function call. For example to create a virtual network client:
    ...
    Set `baseURI` to the `ResourceManagerURL` value used in step 2.

    Set `subscriptionID` to the subscription ID value from step 3.

    To create token see Authentication section below

7. Invoke API methods by using the client that you created in the previous step. For example, to create a virtual network by using the client from previous step:
    ...
    ```
    For a complete example of creating a virtual network on Azure Stack by using Azure Go SDK profile, see Example section below.

# Authentication
To get the `Authorizer` property from Azure Active Directory using Azure Go SDK, install the Go-AutoRest modules. These modules should have been already installed with the Azure Go SDK installation; if not, install the [authentication package](https://github.com/Azure/go-autorest/tree/master/autorest/adal)
The `Authorizer` must be set as the "authorizer" property of the resource client. There are different methods to get an Authorizer; for a complete list see [here](https://github.com/Azure/go-autorest/tree/master/autorest/adal#acquire-access-token).
...
3. Create an OAuthConfig by using NewOAuthConfig method from `adal` module. 
    ...
    Set `activeDirectoryEndpoint` to the value of `loginEndpoint` property that is retreived from the resource manager metadata endpoint on the previous section of this document.
...
4. Finally, create a service principal token by using NewServicePrincipalToken method from `adal` module.
    ...
    Set `tokenAudience` to one of the values in the `audience` list that is retrieved from the resource manager metadata endpoint on the previous section of this document.

    Set `clientID` to the service principal application ID saved when service principal was created on step 1.

    Set `clientSecret` to the service principal application Secret saved when service principal was created on step 1.
...
2. Define your environment variables. Note that to create a virtual network you need to have a resource group.
    ...
        tokenAudience = "yourAudienceFromResourceManagerUrlMetadata"
